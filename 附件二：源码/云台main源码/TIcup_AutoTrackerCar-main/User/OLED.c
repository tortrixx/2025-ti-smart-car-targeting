/***********************************
	本驱动文件仅适配HAL库版本
***********************************/
#include "stm32h7xx_hal.h"	//链接HAL库，你的是什么芯片就改成什么
#include "OLED_Font.h"	 //字库文件
#include "OLED.h"		//声明
#include "i2c.h"

#define i2c_set hi2c2
uint8_t CMD_Data[]={
...
/**
  ...
  * @retval 无
  */
void OLED_ShowStr1(uint8_t x,uint8_t y,int num,uint8_t len,uint8_t size2)
{
	uint8_t t,temp;
	uint8_t enshow=0;	
if(num>0)
...
//显示一个字符号串
//x,y为坐标，chr为字符串，Char_Size为单个字母货符号占用的像素长度
void OLED_ShowStr(uint8_t x,uint8_t y,uint8_t *chr,uint8_t Char_Size)
{
	unsigned char j=0;
	while (chr[j]!='\0')
...
//显示汉字
//hzk 用取模软件得出的数组
//x,y为坐标，no为OLED_Font.h中的第202行开始的数组中的汉字的顺序号
