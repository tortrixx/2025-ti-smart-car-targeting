...
void pid_VA(void)
{
	...
	error_v_A=Tv_A-v_A;
	pid_out_VA=100*(error_v_A-last_error_v_A)+100*error_v_A;
...
void pid_VB(void)//300 75 100hz  200 95 200hz 100 100 500hz
{
	...
	error_v_B=Tv_B-v_B;
	pid_out_VB= 100*(error_v_B-last_error_v_B)+100*error_v_B;
...
/*
void cesu(TIM_HandleTypeDef *htim)
{
	if (htim == &htim3)
  {
		if(v_A>=0)
...
void motor_handle()
{
		...
		v_A=-(z_A-lz_A)*80*500/5000;
		v_B=-(z_B-lz_B)*80*500/5000;
			fffff++;
		if(fffff>2)
		{
			pid_VA();
...
void SET_T_distence(float A,float B)
{
	T_distence_A=A;
	T_distence_B=B;
}
