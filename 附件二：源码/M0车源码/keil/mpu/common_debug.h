...
#define DEBUG_OUTPUT_LEVEL_LOG          ( 1 )                                   //wgx_log 调试输出等级 此定义仅仅输出检查报错 不会直接中断程序
#define DEBUG_OUTPUT_LEVEL_ASSERT       ( 2 )                                   // 断言调试输出等级 此定义在检查报错后会输出wgx_log 并直接中断程序
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Part   End <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

// 此处定义 各层级调试输出等级 用户可以根据自己的需要进行修改
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Part Start >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#define  DEBUG_OUTPUT_LEVEL_DRIVER    ( DEBUG_OUTPUT_LEVEL_ASSERT )           // 芯片外设驱动层调试输出等级 默认断言设置
#define  DEBUG_OUTPUT_LEVEL_DEVICE    ( DEBUG_OUTPUT_LEVEL_ASSERT )           // 外接设备驱动层调试输出等级 默认断言设置
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Part   End <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#if (DEBUG_OUTPUT_LEVEL_DISABLE ==  DEBUG_OUTPUT_LEVEL_DRIVER)
#define debug_message_driver_handler(pass, str) (pass ?  NO_ERROR : ! NO_ERROR)
#else
#define debug_message_driver_handler(pass, str) (pass ?  NO_ERROR : debug_message_handler( DEBUG_OUTPUT_LEVEL_DRIVER, (pass), (str),  FILE_MESSAGE,  LINE_MESSAGE))
#endif
#if (DEBUG_OUTPUT_LEVEL_DISABLE ==  DEBUG_OUTPUT_LEVEL_DEVICE)
#define debug_message_device_handler(pass, str) (pass ?  NO_ERROR : ! NO_ERROR)
#else
#define debug_message_device_handler(pass, str) (pass ?  NO_ERROR : debug_message_handler( DEBUG_OUTPUT_LEVEL_DEVICE, (pass), (str),  FILE_MESSAGE,  LINE_MESSAGE))
#endif
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Part   End <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// 返回参数     uint8            NO_ERROR - 0 - 正常 /  ! NO_ERROR - !0 - 异常
// 使用示例      assert(pass_flag);
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
#define  assert(pass)     (pass ?  NO_ERROR : debug_message_handler(DEBUG_OUTPUT_LEVEL_ASSERT, (pass), (NULL),  FILE_MESSAGE,  LINE_MESSAGE))
...
// 返回参数     uint8            NO_ERROR - 0 - 正常 /  ! NO_ERROR - !0 - 异常
// 使用示例     wgx_log(pass_flag, "Pass flag set.");
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
#define wgx_log(pass, str)   (pass ?  NO_ERROR : debug_message_handler(DEBUG_OUTPUT_LEVEL_LOG, (pass), (str),  FILE_MESSAGE,  LINE_MESSAGE))
...
// 使用示例     debug_message_handler( DEBUG_FILE, 0, "Error",  DEBUG_FILE,  DEBUG_LINE);
// 备注信息     此部分不允许用户更改
//              推荐使用  commmon_debug.h 中的 wgx_log(x, str) 接口
//-------------------------------------------------------------------------------------------------------------------
